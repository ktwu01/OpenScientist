...
这是一份 problem statement。它的具体目的是把一件事说清楚——ResearchSkills 的 `/researchskills-extract` v2 应当被设计成什么样——同时把这件事所依赖的几个前提，按七个连续问题摊开来。
...
下面是 `/researchskills-extract` v2 的具体设计。它把上面这个目标拆成四个组件——A、C、B 和一个最终的聚类层——并由一个 Step 0 的纸驱动语义筛选作为入口。
