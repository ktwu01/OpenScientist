#!/usr/bin/env node
/**
 * extract-skills.js — Codex CLI version
 *
 * Orchestrates per-session skill extraction using Codex CLI.
 * For each uncached session: format → call `codex exec` → validate.
 *
 * This script is the deterministic loop; Codex does the AI judgment.
 ...
 *   --concurrency <n>          Parallel codex exec calls per batch (default: 5)
...
const UTILS_DIR = path.join(os.homedir(), '.codex', 'skills', 'extract-knowhow', 'scripts');
...
// ---------------------------------------------------------------------------
// Async codex runner (non-blocking)
//
// NOTE: Uses spawn() which is safe against shell injection (no shell involved).
// The prompt is written to a temp file and piped via stdin to avoid arg-length
// limits. The temp file is cleaned up on completion, timeout, or error.
// ---------------------------------------------------------------------------

function runCodexAsync(prompt, timeoutMs = 600_000) {
  return new Promise((resolve) => {
    const chunks = [];
    const tmpFile = path.join(os.tmpdir(), `codex-prompt-${Date.now()}-${Math.random().toString(36).slice(2)}.txt`);
...
// ---------------------------------------------------------------------------
// Parse skill-md blocks from Codex output → write files → validate
// ---------------------------------------------------------------------------
...
function injectProjectMeta(content, projectMeta) {
  ...
  const insert = [];
  // Inject fixed fields that Codex no longer outputs
...
// ---------------------------------------------------------------------------
// Parse Codex output and collect result
// ---------------------------------------------------------------------------
...
async function main() {
  ...
  console.log(`Concurrency: ${opts.concurrency} parallel Codex calls\n`);
  ...
  console.log(`\nPhase 2: Extracting skills (${prepared.length} sessions → ${workUnits.length} Codex calls, ${opts.concurrency} at a time)...`);
  const results = [];
  let completed = 0;
  ...
  for (let i = 0; i < workUnits.length; i += opts.concurrency) {
    ...
    const promises = chunk.map(async ({ sid, prompt, segFile, segInfo, projectMeta }) => {
      ...
      const { ok, output, error } = await runCodexAsync(prompt, 300_000);
    ...
    if (opts.singleBatch) {
      ...
      console.log(`\n  --single-batch: stopping after batch ${batchNum}. ${remainingCalls} Codex calls remaining.`);
      break;
    }
  }
  ...
  // Codex-reported totals (what Codex claimed to extract)
  const codexTotal = totals.episodic + totals.semantic + totals.procedural;
...
═══════════════════════════════════════════════
  ...
  Codex reported:     ${codexTotal} skills (E:${totals.episodic} S:${totals.semantic} P:${totals.procedural})
