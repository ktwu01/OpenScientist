 ...
 * score-skills.js
 *
 * Stage 5 of /extract-knowhow: score cleaned skills with Codex.
 * Spawns a Codex CLI instance (full-auto, ephemeral) that reads
...
- **Core criterion**: Without this skill, would the strongest AI (GPT-5.4-level) perform worse in the corresponding dimension?
...
async function main() {
  ...
  console.log('Spawning Codex for scoring...\n');
  const prompt = buildPrompt(files);
  const { ok, output, error } = await runCodex(prompt, opts.verbose);

  if (!ok) {
    console.error(`\nCodex scoring failed: ${error}`);
    process.exit(1);
  }
...
═══════════════════════════════════════════════`);
    ...
    console.log('\nCodex completed but no SCORE_SUMMARY found in output.');
    if (!opts.verbose) {
